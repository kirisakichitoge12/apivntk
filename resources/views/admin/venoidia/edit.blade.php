...
<head>
    ...
    <title>Chỉnh sửa Mô tả Địa Điểm - Admin Panel</title>
    ...
    <div class="admin-container">
        <div class="admin-card">
            <div class="admin-header">
                <h1 class="admin-title">Chỉnh sửa Mô tả Địa Điểm</h1>
                <a href="{{ route('venoidia.index') }}" class="btn btn-cancel">
                    <i class="fas fa-arrow-left me-2"></i>Quay lại
                </a>
            </div>
            
            <form action="{{ route('venoidia.update', $venoidia) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" id="title" class="form-control" 
                           value="{{ old('title', $venoidia->title) }}" 
                           placeholder="Nhập tiêu đề địa điểm" required maxlength="255">
                    <div class="character-count">
                        <span id="title-count">{{ strlen(old('title', $venoidia->title)) }}</span>/255 ký tự
                    </div>
                    @error('title')
                ...
                <div class="mb-4">
                    <label for="editor" class="form-label">Mô tả nội dung</label>
                    <textarea id="editor" name="description" class="form-control" rows="10" 
                              placeholder="Nhập mô tả chi tiết về địa điểm">{!! old('description', $venoidia->description) !!}</textarea>
                    @error('description')
                        <div class="text-danger mt-2">{{ $message }}</div>
                    @enderror
                </div>
